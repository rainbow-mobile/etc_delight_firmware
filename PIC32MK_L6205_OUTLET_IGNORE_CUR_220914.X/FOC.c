...
//        IOCON1bits.OVRENL = 0;
        if(vref > 0){
            ...
            IOCON1bits.OVRDAT = 0b00;
            PDC1 = vref*PWM_RES;
//            PDC1 = vref*PWM_RES/2+PWM_RES/2;
        }else if(vref < 0){
            IOCON1bits.OVRENH = 1;
            IOCON1bits.OVRENL = 0;
            IOCON1bits.OVRDAT = 0b00;
            PDC1 = PWM_RES+vref*PWM_RES;
//            PDC1 = vref*PWM_RES/2+PWM_RES/2;
        }else{
            IOCON1bits.OVRENH = 1;
            IOCON1bits.OVRENL = 1;
...
void FOC_CurrentControl_Double(int ch){
    ...
    Vq = (float)(ref_current_q_modi) * 15.0 / 48000.0;
    if(Vq >= 1.0)
        Vq = 1.0;
    if(Vq <= -1.0)
        Vq = -1.0;
    ...
    int temp_error = (REF[ch] - NewPos[ch])/ENC_MUL;
    measured_current_q[ch] = Pgain_pos[ch]*temp_error/5;//25;
    return;
    // Ignore Current Sensor
    ...
    if(ch == 0){
        ...
        Current_R_filt[ch] = 0.95*Current_R_filt[ch] + 0.05*Current_R[ch];
        I_q = CURRENT_CONSTANT * (float)(Current_R[ch]);
//        I_q = CURRENT_CONSTANT * (float)(Current_R_filt[ch]);
...
//        measured_current_q[ch] = CURRENT_CONSTANT * (float)(Current_R_filt[ch]);
    }else{
        Current_R[ch] = (ADCDATA1 - RZ[ch]);
        Current_R_filt[ch] = 0.95*Current_R_filt[ch] + 0.05*Current_R[ch];
        measured_current_q[ch] = CURRENT_CONSTANT * (float)(Current_R[ch]);
