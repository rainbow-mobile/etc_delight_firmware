...
//    PTPER = 3000;
    ...
    IOCON5bits.PMOD = 0b00;     // complementary
    IOCON5bits.PENH = 1;        // PWM (not GPIO)
    IOCON5bits.PENL = 0;        // GPIO
    IOCON5bits.FLTMOD = 0b11;   // fault disable
    IOCON5bits.POLH = 0;        // Active High
    IOCON5bits.POLL = 0;        // Active High
    
    
    IOCON1bits.OVRENH = 1;  // pwm          - pulse
    IOCON1bits.OVRENL = 1;  // override     - pulse
    IOCON2bits.OVRENH = 1;  // pwm          - pulse
    IOCON2bits.OVRENL = 1;  // override     - pulse
    IOCON3bits.OVRENH = 0;  // pwm          - pulse
    IOCON3bits.OVRENL = 0;  // override     - pulse
    IOCON4bits.OVRENH = 0;  // pwm          - pulse
    IOCON4bits.OVRENL = 0;  // override     - pulse
    IOCON4bits.OVRENH = 0;  // pwm          - pulse
    IOCON4bits.OVRENL = 0;  // override     - pulse
...
//    ALTDTR4 = 20;       // 8*20 = 160ns
    ...
    PDC1 = PWM_STOP;
    PDC2 = PWM_STOP;
    PDC3 = PWM_STOP; //(int)(PWM_RES*1);//PWM_RES/32;   //PWM_STOP;
    PDC4 = PWM_STOP; //(int)(PWM_RES*1);//PWM_RES/3;   //PWM_STOP;
    PDC5 = PWM_STOP; //(int)(PWM_RES*1);//PWM_RES/32;   //PWM_STOP;
}
